...
ReBit은 선택한 Whisper 기반 음성 인식 모델을 기기로 다운로드하고, 가져온 오디오를 기기 안에서 분석하여 스크립트를 생성합니다. 오디오 내용은 스크립트 생성을 위해 ReBit이 운영하는 서버 또는 모델 호스팅 서비스로 전송되지 않습니다.

음성 인식 모델을 다운로드할 때는 네트워크 통신이 발생합니다. 현재 모델 파일은 WhisperKit을 통해 제3자 모델 호스팅 서비스에서 제공되며, 해당 서비스는 다운로드 요청 과정에서 IP 주소와 일반적인 네트워크 요청 정보를 처리할 수 있습니다.
...
현재 버전은 비맞춤형 광고를 요청하고, 광고 개인화를 비활성화하며, 앱 간 추적 목적으로 사용자의 광고 식별자에 접근하기 위한 App Tracking Transparency 권한을 요청하지 않습니다.
...
앱 기능 제공에 필요한 범위에서 다음 서비스 제공자가 정보를 처리할 수 있습니다.
...
ReBit downloads the selected Whisper-based speech-recognition model to the device and analyzes imported audio on the device to generate transcripts. Audio content is not sent to a ReBit-operated server or to the model hosting service for transcription.

Downloading a speech-recognition model requires network communication. The model files are currently provided through WhisperKit by a third-party model hosting service, which may process the user's IP address and standard network request information during the download.
...
The following service providers may process information as needed to provide app functionality:
